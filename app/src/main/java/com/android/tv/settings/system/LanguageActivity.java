...
public class LanguageActivity extends DialogActivity implements ActionAdapter.Listener {
    ...
    /**
     * MichaelKoo add by 20160812
     * @param src
     * @return
     */
    private static ArrayAdapter<LocalePicker.LocaleInfo> filter(ArrayAdapter<LocalePicker.LocaleInfo> src){

        java.util.List<LocalePicker.LocaleInfo> dataList=new ArrayList<LocalePicker.LocaleInfo>();
        for(int in=0,size=src.getCount();in<size;in++){
            dataList.add(src.getItem(in));
        }
        java.util.List<LocalePicker.LocaleInfo> copy=new ArrayList<LocalePicker.LocaleInfo>(dataList);
        for(LocalePicker.LocaleInfo info:copy){
            Locale locale=info.getLocale();

            if(locale.equals(Locale.US)||locale.equals(Locale.SIMPLIFIED_CHINESE)){
                System.out.println("filter  not ="+locale);

            }else{
                System.out.println("filter="+locale);
                dataList.remove(info);
            }
        }
        src.clear();
        src.addAll(dataList);
        System.out.println("us="+Locale.US+" ,zh="+Locale.SIMPLIFIED_CHINESE);
    ...
    private void init() {
        ...
        System.out.println("step 1 "+mLocales.getCount());

        mLocales=filter(mLocales);
        System.out.println("step 2 "+mLocales.getCount());
