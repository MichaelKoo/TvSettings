...
public class DateTimeActivity extends BaseSettingsActivity implements ActionAdapter.Listener {
    ...
    // MStar Android Patch Begin
    private String[] extraTimeZones = {"Australia/West"};
    // MStar Android Patch End
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // MStar Android Patch Begin
        Intent tvTimeIntent = new Intent(MIntent.ACTION_TV_AUTO_TIME_SYNC);
        tvTimeIntent.putExtra(MIntent.EXTRA_KEY_TV_AUTO_TIME, getAutoState(Settings.Global.TV_AUTO_TIME));
        sendBroadcast(tvTimeIntent);
        // MStar Android Patch End
    ...
    private void setAutoDateTime(boolean on) {
        ...
        // MStar Android Patch Begin
        if (on) {
            Settings.Global.putInt(getContentResolver(), Settings.Global.TV_AUTO_TIME, 0);
        }
        // MStar Android Patch End
    }

    // MStar Android Patch Begin
    private void setTvAutoDateTime(boolean on) {
        Settings.Global.putInt(getContentResolver(), Settings.Global.TV_AUTO_TIME, on ? 1 : 0);
        if (on) {
            Settings.Global.putInt(getContentResolver(), Settings.Global.AUTO_TIME, 0);
        }
        Intent tvTimeIntent = new Intent(MIntent.ACTION_TV_AUTO_TIME_SYNC);
        tvTimeIntent.putExtra(MIntent.EXTRA_KEY_TV_AUTO_TIME, on);
        sendBroadcast(tvTimeIntent);
    }
    // MStar Android Patch End
    ...
    public void onActionClicked(Action action) {
        ...
        switch (behavior) {
            case ON:
                ...
                // MStar Android Patch Begin
                } else if (mState == ActionType.AUTO_TV_DATE_TIME) {
                    setTvAutoDateTime(true);
                // MStar Android Patch End
                }
                goBack();
                break;
            ...
            if (adapter != null) {
                ...
                switch ((ActionType) mState) {
                    case DATE_TIME_OVERVIEW:
                        ...
                        // MStar Android Patch Begin
                        mActions.add(ActionType.AUTO_TV_DATE_TIME.toAction(mResources,
                                mHelper.getStatusStringFromBoolean(getAutoState(
                                        Settings.Global.TV_AUTO_TIME))));
                        // MStar Android Patch End
    ...
    protected void refreshActionList() {
        ...
        //MStar Android Patch Begin
        boolean tvAutoTime = getAutoState(Settings.Global.TV_AUTO_TIME);
        //MStar Android Patch End
        switch ((ActionType)mState) {
            case DATE_TIME_OVERVIEW:
                mActions.add(ActionType.AUTO_DATE_TIME.toAction(mResources,
                ...
                //MStar Android Patch Begin
                mActions.add(ActionType.AUTO_TV_DATE_TIME.toAction(mResources,
                        mHelper.getStatusStringFromBoolean(tvAutoTime)));
                //MStar Android Patch End
                mActions.add(ActionType.DATE.toAction(mResources, mNowDate));
                mActions.add(ActionType.TIME.toAction(mResources, mNowTime));
                break;
            case DATE:
                //MStar Android Patch Begin
                mActions.add(ActionType.DATE_SET_DATE.toAction(mResources, mNowDate, ((!autoTime) && (!tvAutoTime))));
                //MStar Android Patch End
                mActions.add(ActionType.DATE_CHOOSE_FORMAT.toAction(mResources,
                        getDateFormatExampleString()));
                break;
            case TIME:
                //MStar Android Patch Begin
                mActions.add(ActionType.TIME_SET_TIME.toAction(mResources, mNowTime, ((!autoTime) && (!tvAutoTime))));
                //MStar Android Patch End
            ...
            case AUTO_TV_DATE_TIME:
                mActions.add(ActionBehavior.ON.toAction(ActionBehavior.getOnKey(
                        ActionType.AUTO_TV_DATE_TIME.name()), mResources, tvAutoTime));
                mActions.add(ActionBehavior.OFF.toAction(ActionBehavior.getOffKey(
                        ActionType.AUTO_TV_DATE_TIME.name()), mResources, !tvAutoTime));
                break;
            // MStar Android Patch End
    ...
    private ArrayList<TimeZoneInfo> getTimeZones(Context context) {
        ...
        // MStar Android Patch Begin
        for (String id : extraTimeZones) {
            TimeZone tz = TimeZone.getTimeZone(id);
            if (tz != null) {
                timeZones.add(new TimeZoneInfo(id, tz.getDisplayName(), tz.getOffset(date)));
            }
        }
        // MStar Android Patch End
        return timeZones;
    ...
    // MStar Android Patch Begin
    /**
     * Get system's installed time zone information into an array of TimeZoneInfo
     * objects. Used for debug purpose.
     */
    private ArrayList<TimeZoneInfo> getAvailableTimeZones() {
        ArrayList<TimeZoneInfo> timeZones = new ArrayList<TimeZoneInfo>();
        final long date = Calendar.getInstance().getTimeInMillis();
