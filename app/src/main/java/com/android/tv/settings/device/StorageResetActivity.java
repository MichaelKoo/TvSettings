...
public class StorageResetActivity extends SettingsLayoutActivity {
	...
	/**
	 * Support for shutdown-after-reset. If our launch intent has a true value
	 * for the boolean extra under the following key, then include it in the
	 * intent we use to trigger a factory reset. This will cause us to shut down
	 * instead of restart after the reset.
	 */
	...
	private final MeasurementReceiver mReceiver = new MeasurementReceiver() {
		...
		@Override
		public void updateApproximate(StorageMeasurement meas, long totalSize,
				long availSize) {
			if (mLastMeasurementDetails == null) {
				StorageResetActivity.this.updateApproximate(totalSize,
						availSize);
			}
		}

		@Override
		public void updateDetails(StorageMeasurement meas,
	...
	private class StorageDrawableGetter extends DrawableGetter {
		...
		void setDrawable(ArrayList<PercentageBarChart.Entry> entries) {
			mDrawable = new PercentageBarChart(entries,
					mRes.getColor(R.color.storage_avail),
	...
	private void updateStorageSize(long availSize, long appsSize,
			long dcimSize, long musicSize, long downloadsSize, long cacheSize,
			long miscSize) {
	...
	void updateStorageDescription(long totalSize,
	...
	private void updateApproximate(long totalSize, long availSize) {
		...
		entries.add(new PercentageBarChart.Entry(0, usedSize
				/ (float) totalSize, android.graphics.Color.GRAY));

		updateStorageSize(availSize, INVALID_SIZE, INVALID_SIZE, INVALID_SIZE,
				INVALID_SIZE, INVALID_SIZE, INVALID_SIZE);
		updateStorageDescription(totalSize, entries);
	}

	private void addEntry(List<PercentageBarChart.Entry> entries,
			StorageItem storageItem, long size, long totalSize) {
		if (size > 0) {
			entries.add(new PercentageBarChart.Entry(storageItem.ordinal(),
					size / (float) totalSize, storageItem
	...
	private void updateDetails(MeasurementDetails details) {
		...
		addEntry(entries, StorageItem.PICTURES_VIDEO, dcimSize,
				details.totalSize);
		addEntry(entries, StorageItem.AUDIO, musicSize, details.totalSize);
		addEntry(entries, StorageItem.DOWNLOADS, downloadsSize,
				details.totalSize);
		addEntry(entries, StorageItem.CACHED_DATA, details.cacheSize,
				details.totalSize);
		addEntry(entries, StorageItem.MISC, details.miscSize, details.totalSize);

		Collections.sort(entries);

		updateStorageSize(details.availSize, details.appsSize, dcimSize,
				musicSize, downloadsSize, details.cacheSize, details.miscSize);
