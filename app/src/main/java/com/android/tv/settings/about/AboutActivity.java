...
import com.android.tv.settings.update.SystemLocalUpdateActivity;
import com.android.tv.settings.update.SystemNetUpdateActivity;
...
public class AboutActivity extends DialogActivity implements ActionAdapter.Listener,
    ...
    // MStar Android Patch Begin
    /**
     * Action key for system update.
     */
    private static final String KEY_SYSTEM_UPDATE = "about_system_update";
    // MStar Android Patch End
    ...
    public void onActionClicked(Action action) {
        ...
        // MStar Android Patch Begin
        } else if (TextUtils.equals(key, KEY_SYSTEM_UPDATE)) {
            ArrayList<Action> actions = getSystemUpdateActions();
            setContentAndActionFragments(ContentFragment.newInstance(
                    getString(R.string.about_system_update), null, null),
                    ActionFragment.newInstance(actions));
        // MStar Android Patch End
        } else {
    ...
    // MStar Android Patch Begin
    private ArrayList<Action> getSystemUpdateActions() {
        ArrayList<Action> actions = new ArrayList<Action>();
        Intent local_update_intent = new Intent();
        local_update_intent.setClass(this, SystemLocalUpdateActivity.class);
        Intent net_update_intent = new Intent();
        net_update_intent.setClass(this, SystemNetUpdateActivity.class);
        actions.add(new Action.Builder()
                .key("local_update")
                .title(getString(R.string.system_local_update))
                .intent(local_update_intent)
                .build());
        actions.add(new Action.Builder()
                .key("net_update")
                .title(getString(R.string.system_net_update))
                .intent(net_update_intent)
                .build());
        return actions;
    }
    // MStar Android Patch End
    ...
    private ArrayList<Action> getActions() {
        ...
        // gavin add with hardware info 20160726 start
        Intent hardInfoIntent = new Intent();
        hardInfoIntent.setClass(this, HardwareInfoActivity.class);
        actions.add(new Action.Builder()
                .key("name")
                .title(getString(R.string.hardware_info))
                .intent(hardInfoIntent)
                .build());
        // gavin add with hardware info 20160726 start
