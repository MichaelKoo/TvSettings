...
final class LocalBluetoothProfileManager {
     ...
     */
    // proting from brcm
    synchronized void updateLocalProfiles(ParcelUuid[] uuids) {

        // A2DP - Note there can be only one role for the profile. Therefore only
        //             a single profile will be added for either.
    ...
    synchronized void updateProfiles(ParcelUuid[] uuids, ParcelUuid[] localUuids,
        ...
        // proting from brcm
        if (mA2dpProfile != null) {
            if (BluetoothUuid.isUuidPresent(localUuids, BluetoothUuid.AudioSource) &&
        ...
        if (mA2dpProfile != null) {
            if (BluetoothUuid.isUuidPresent(localUuids, BluetoothUuid.AudioSink) &&
                BluetoothUuid.containsAnyUuid(uuids, A2dpProfile.SOURCE_UUIDS)) {
                profiles.add(mA2dpProfile);
                removedProfiles.remove(mA2dpProfile);
            }
        }
